...
Router.route('/versions/:_id?', {
  name: 'versions',
  waitOn: function () {
    return [Meteor.subscribe('surveyVersions'), Meteor.subscribe('schoolnames', this.params._id)];
  },
  action: function () {
...
Router.route('/survey/:_id?', {
  ...
  data: function () {
    var data = {
      survey_version_id : this.params._id,
    };
    return data;
  },
});
...
Router.route('/labs/edit/:id', {
  ...
  data: function() {
    var data = {
      survey_version_id : Session.get('selectedSurveryVersionId'),
    };
    return data;
  }
});
